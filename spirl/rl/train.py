...
class RLTrainer:
    ...
    def get_config(self):
        ...
        # wandb config
        if hasattr(conf_module, 'wandb'):
            conf.wandb_project_name  = conf_module.wandb.wandb_project_name
            conf.wandb_entity_name   = conf_module.wandb.wandb_entity_name
            if hasattr(conf_module.wandb, 'wandb_group'):
                conf.wandb_group     = conf_module.wandb.wandb_group
            else:
                conf.wandb_group     =  None
        else:
            conf.wandb_project_name  = WANDB_PROJECT_NAME
            conf.wandb_entity_name   = WANDB_ENTITY_NAME
            conf.wandb_group         = None
            ...
            if self.args.mode == 'train':
                ...
                if self._hp.logging_target == 'wandb':
                    logger = WandBLogger(exp_name, WANDB_PROJECT_NAME, entity=WANDB_ENTITY_NAME,
                                         path=self._hp.exp_path, conf=conf, group_name=self.conf.wandb_group)
                else:
